...
class FilesystemStore {
  ...
  getObject(bucket, key, options, done) {
    ...
    putObject(object, done) {
      ...
      if(object.content.file) {
        const fileBuffer = object.content.file.buffer;
        const md5 = crypto.createHash("md5").update(fileBuffer).digest("hex");

        fs.writeFile(path.join(dirName, CONTENT_FILE), fileBuffer);
        ...
        this.storeMetadata(
            ...
            err => {
                if (err) return done(err);
                done(null, md5, fileBuffer.length);
            }
        );
      } else {
